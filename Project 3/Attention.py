...
class AttentionUtils:
    ...
    def mask_loss(loss_func, mask_tok):
        """
        Mask the loss function to ignore padding tokens
        """
        def masked_loss(y_true, y_pred):
            mask = tf.math.not_equal(y_true, mask_tok)
    ...
    def mask_accuracy(acc_func, mask_tok):
        """
        Mask the accuracy function to ignore padding tokens
        """
        def m_acc(y_true, y_pred):
            mask = tf.cast(tf.math.not_equal(y_true, mask_tok), tf.float32)
...
class MultiHeadAttention(tf.keras.layers.Layer):
    def __init__(self, d_model, num_heads, attn_func=None):
        super(MultiHeadAttention, self).__init__()
        self.num_heads = num_heads
        self.d_model = d_model
        if attn_func is None:
            self.attn_func = AttentionUtils.scaled_dot_product_attention
        else:
    ...
    def call(self, v, k, q, mask):
        ...
        scaled_attention, attention_weights = self.attn_func(q, k, v, mask)
...
class EncoderLayer(tf.keras.layers.Layer):
    def __init__(self, d_model, num_heads, dff, rate=0.1, attn_func=None):
        super(EncoderLayer, self).__init__()

        self.mha = MultiHeadAttention(d_model, num_heads, attn_func)
...
class DecoderLayer(tf.keras.layers.Layer):
  def __init__(self, d_model, num_heads, dff, rate=0.1, attn_func=None):
    ...
    self.mha1 = MultiHeadAttention(d_model, num_heads, attn_func)
    self.mha2 = MultiHeadAttention(d_model, num_heads, attn_func)
...
class Encoder(tf.keras.layers.Layer):
  def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size,
               maximum_position_encoding, embedding_weights=None, freeze_embedding=False, rate=0.1, attn_func=None):
    ...
    self.enc_layers = [EncoderLayer(d_model, num_heads, dff, rate, attn_func)
...
class Decoder(tf.keras.layers.Layer):
  def __init__(self, num_layers, d_model, num_heads, dff, target_vocab_size,
               maximum_position_encoding, rate=0.1, attn_func=None):
    ...
    self.dec_layers = [DecoderLayer(d_model, num_heads, dff, rate, attn_func)
                       for _ in range(num_layers)]
    self.dropout = tf.keras.layers.Dropout(rate)
...
class Transformer(tf.keras.Model):
  def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size,
               target_vocab_size, pe_input, pe_target, rate=0.1, pad_token=0, attn_func=None):
    super().__init__()

    if type(attn_func) == list or type(attn_func) == tuple:
        assert len(attn_func) == 2
        self.encoder_attn_func = attn_func[0]
        self.decoder_attn_func = attn_func[1]
    else:
        self.encoder_attn_func = attn_func
        self.decoder_attn_func = attn_func


    self.encoder = Encoder(num_layers, d_model, num_heads, dff,
                           input_vocab_size, pe_input, rate, self.encoder_attn_func)

    self.decoder = Decoder(num_layers, d_model, num_heads, dff,
                           target_vocab_size, pe_target, rate, self.decoder_attn_func)
...
class EncoderModel(tf.keras.Model):
    ...
    def create_masks(self, inp):
        # Encoder padding mask
        enc_padding_mask = AttentionUtils.create_look_ahead_mask(inp.shape[1])
